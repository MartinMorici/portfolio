...
const Skills = () => {
    const { mouseLeaveSectionss, mouseEnterText, mouseEnterButton, reveal} = useContext(CursorContext)
  return (
            ...
            <Container className='z-[1]'>
                ...
                <div className='  grid-cols-[repeat(2,1fr)] skills:grid-cols-[repeat(2,45%)] md:grid-cols-[repeat(2,280px)] lg:grid-cols-[repeat(3,280px)] xl:grid-cols-[repeat(4,280px)] grid gap-3 mt-7 place-content-center'  onMouseEnter={mouseEnterText} onMouseLeave={mouseLeaveSectionss}>
                    {skills.map((skill,index) => {
                        return(
                            <motion.div viewport={{once:true}} initial={{opacity:0, y:50}} whileInView={{opacity:1,y:0}} transition={{type:'spring', delay:index * .04}} className='group' key={index}>
