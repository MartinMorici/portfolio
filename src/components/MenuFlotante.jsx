...
const MenuFlotante = () => {
  ...
  return (
    <nav className={`hidden  h-[65px] bg-black bg-opacity-10 backdrop-filter backdrop-blur-lg nav:flex items-center justify-center w-full fixed top-0 z-50 bg-transparent tracking-[-1px] text-[#ffffff] font-semibold font-roboto ${show ? 'translate-y-0 transition-[transform] duration-500' : 'translate-y-[-100%] transition-[transform] duration-500'}`} onMouseEnter={mouseEnterText} onMouseLeave={mouseLeaveSectionss}>
      <ul className={` w-fit mx-auto flex flex-col nav:flex-row gap-12 text-[1.8rem]  nav:text-[17px] whitespace-nowrap`}>
